...
export class OperadorPluckComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.myData$.pipe(pluck("mails", "gmail")).subscribe(console.log);
    ...
    console.log(
      "*************METODO DISTINCTUNTILCHANGED AND DEBOUNCETIME*********"
    );
    ...
    heroes$
      .pipe(map<string, string>(capitalizar))
  ...
  exampleScan(): void {
    const buildingObject = this.subjectScan$.pipe(
      scan((acc, curr) => Object.assign({}, acc, curr), {})
    );

    buildingObject.subscribe((val) =>
      console.log(
        "%cSCAN EXAMPLE ",
    ...
    this.subjectScan$.next({ name: "Pepe" });
    this.subjectScan$.next({ age: 30 });
    this.subjectScan$.next({ city: "Valencia" });
    this.subjectScan$.next({ age: 33 });
  }
  ...
  examplePluckOperator(): void {
    ...
    letterKeup$.pipe(pluck("key")).subscribe((code) => {
      element?.append(code);
    });
    letterKeup$.pipe(pluck("target", "baseURI")).subscribe((baseUri: any) => {
      if (!elementUrl?.innerHTML) {
        elementUrl?.append(baseUri);
      }
    });
    letterKeup$.pipe(mapTo("devuelve un valor")).subscribe((res) => {
